...
namespace Catalyst.Engine.Physics
{
    ...
    public class QuadTree
    {
        ...
        private int GetIndex(Collider2D collider)
        {
            ...
            bool topQuadrant = (collider.Bounds.Y < horizontalMidpoint && collider.Bounds.Y + collider.Bounds.Height < horizontalMidpoint);
            // Object can completely fit within the bottom quadrants
            bool bottomQuadrant = (collider.Bounds.Y > horizontalMidpoint);

            // Object can completely fit within the left quadrants
            if (collider.Bounds.X < verticalMidpoint && collider.Bounds.X + collider.Bounds.Width < verticalMidpoint)
            {
                if (topQuadrant)
                {
                    index = 1;
            ...
            else if (collider.Bounds.X > verticalMidpoint)
            {
                if (topQuadrant)
                {
                    index = 0;
        ...
        public void Retrieve(List<Collider2D> returnObjects, Collider2D pRect)
        {
            ...
            foreach (Collider2D c in _objects)
            {
                returnObjects.Add(c);
            }
        }
