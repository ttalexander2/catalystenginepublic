...
namespace Catalyst
{
    ...
    public class MemoryEditor
    {
        ...
        {
            ImGuiNET.ImGui.SetNextWindowSize(new Vector2(500, 350), ImGuiCond.FirstUseEver);
            ...
            ImGuiNET.ImGui.BeginChild("##scrolling", new Vector2(0, -ImGuiNative.igGetFrameHeightWithSpacing()), false, 0);
            ...
            float glyph_width = ImGuiNET.ImGui.CalcTextSize("F").X;
            ...
            {
                if (ImGuiNET.ImGui.IsKeyPressed(ImGuiNET.ImGui.GetKeyIndex(ImGuiKey.UpArrow)) && DataEditingAddr >= Rows) { DataEditingAddr -= Rows; DataEditingTakeFocus = true; }
                else if (ImGuiNET.ImGui.IsKeyPressed(ImGuiNET.ImGui.GetKeyIndex(ImGuiKey.DownArrow)) && DataEditingAddr < mem_size - Rows) { DataEditingAddr += Rows; DataEditingTakeFocus = true; }
                else if (ImGuiNET.ImGui.IsKeyPressed(ImGuiNET.ImGui.GetKeyIndex(ImGuiKey.LeftArrow)) && DataEditingAddr > 0) { DataEditingAddr -= 1; DataEditingTakeFocus = true; }
                else if (ImGuiNET.ImGui.IsKeyPressed(ImGuiNET.ImGui.GetKeyIndex(ImGuiKey.RightArrow)) && DataEditingAddr < mem_size - 1) { DataEditingAddr += 1; DataEditingTakeFocus = true; }
            }
            ...
            for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible items
            {
                ...
                ImGuiNET.ImGui.Text(FixedHex(base_display_addr + addr, addr_digits_count) + ": ");
                ...
                {
                    ImGuiNET.ImGui.SameLine(line_start_x + cell_width * n);

                    if (DataEditingAddr == addr)
                    {
                        // Display text input on current byte
                        ImGuiNET.ImGui.PushID(addr);
                        ...
                        ImGuiNET.ImGui.PushItemWidth(ImGuiNET.ImGui.CalcTextSize("FF").X);
                        ...
                        if (ImGuiNET.ImGui.InputText("##data", DataInput, 32, flags, callback, (IntPtr)(&cursor_pos)))
                            data_write = data_next = true;
                        else if (!DataEditingTakeFocus && !ImGuiNET.ImGui.IsItemActive())
                            DataEditingAddr = -1;
                    ...
                    {
                        ImGuiNET.ImGui.Text(FixedHex(mem_data[addr], 2));
                        if (AllowEdits && ImGuiNET.ImGui.IsItemHovered() && ImGuiNET.ImGui.IsMouseClicked(0))
                        {
                ...
                ImGuiNET.ImGui.SameLine(line_start_x + cell_width * Rows + glyph_width * 2);
                ...
                ImGuiNET.ImGui.TextUnformatted(asciiVal.ToString());  //use unformatted, so string can contain the '%' character
            }
            ...
            ImGuiNET.ImGui.PushItemWidth(50);
            ImGuiNET.ImGui.PushAllowKeyboardFocus(true);
            int rows_backup = Rows;
            if (ImGuiNET.ImGui.DragInt("##rows", ref Rows, 0.2f, 4, 32, "%.0f rows"))
            {
                if (Rows <= 0) Rows = 4;
                Vector2 new_window_size = ImGuiNET.ImGui.GetWindowSize();
                new_window_size.X += (Rows - rows_backup) * (cell_width + glyph_width);
                ImGuiNET.ImGui.SetWindowSize(new_window_size);
            }
            ImGuiNET.ImGui.PopAllowKeyboardFocus();
            ImGuiNET.ImGui.PopItemWidth();
            ImGuiNET.ImGui.SameLine();
            ImGuiNET.ImGui.Text(string.Format(" Range {0}..{1} ", FixedHex(base_display_addr, addr_digits_count),
                FixedHex(base_display_addr + mem_size - 1, addr_digits_count)));
            ImGuiNET.ImGui.SameLine();
            ImGuiNET.ImGui.PushItemWidth(70);
            if (ImGuiNET.ImGui.InputText("##addr", AddrInput, 32, ImGuiInputTextFlags.CharsHexadecimal | ImGuiInputTextFlags.EnterReturnsTrue, null))
            {
                int goto_addr;
                if (TryHexParse(AddrInput, out goto_addr))
                {
                    ...
                    {
                        ImGuiNET.ImGui.BeginChild("##scrolling");
                        ImGuiNET.ImGui.SetScrollFromPosY(ImGuiNET.ImGui.GetCursorStartPos().Y + (goto_addr / Rows) * ImGuiNative.igGetTextLineHeight());
                        ImGuiNET.ImGui.EndChild();
