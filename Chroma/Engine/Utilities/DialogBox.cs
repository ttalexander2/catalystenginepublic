...
namespace Chroma.Engine.Utilities
{
    public class DialogBox
    {
        ...
        private Vector2 _characterSize = DialogBoxWindow.Instance.DialogFont.MeasureString(new StringBuilder("W", 1));
        ...
        public DialogBox()
        {
            ...
            _fillTexture = new Texture2D(DialogBoxWindow.Instance.GraphicsDevice, 1, 1);
            _fillTexture.SetData(new[] { FillColor });

            _borderTexture = new Texture2D(DialogBoxWindow.Instance.GraphicsDevice, 1, 1);
            _borderTexture.SetData(new[] { BorderColor });

            _pages = new List<string>();
            _currentPage = 0;

            var sizeX = (int)(DialogBoxWindow.Instance.GraphicsDevice.Viewport.Width * 0.5);
            var sizeY = (int)(DialogBoxWindow.Instance.GraphicsDevice.Viewport.Height * 0.2);
        ...
        public void Update()
        {
            if (Active)
            {
                ...
                if ((DialogBoxWindow.Instance.KeyState.IsKeyDown(Keys.Enter) && DialogBoxWindow.Instance.PreviousKeyState.IsKeyUp(Keys.Enter)) ||
                    (DialogBoxWindow.Instance.GamePadState.Buttons.A == ButtonState.Pressed))
                {
                ...
                if ((DialogBoxWindow.Instance.KeyState.IsKeyDown(Keys.X) && DialogBoxWindow.Instance.PreviousKeyState.IsKeyUp(Keys.X)) ||
                    (DialogBoxWindow.Instance.GamePadState.Buttons.X == ButtonState.Pressed))
                {
                    Hide();
                }
            }
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            if (Active)
            {
                ...
                spriteBatch.DrawString(DialogBoxWindow.Instance.DialogFont, _pages[_currentPage], TextPosition, DialogColor);
                ...
                if (BlinkIndicator() || _currentPage == _pages.Count - 1)
                {
                    ...
                    spriteBatch.DrawString(DialogBoxWindow.Instance.DialogFont, ">", indicatorPosition, Color.Red);
                }
            }
        }
